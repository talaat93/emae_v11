...
if ($page) {
    ...
    if ($route === 'electricien-meaux') {
        $electricConfig = service_electric_page_settings($page);
        $resolveHref = static function (string $href): string {
            ...
            if (!preg_match('#^(https?:|tel:|mailto:|/)#i', $href)) {
                return route_url($href);
            }
            return $href;
        };

        $meta = [
            'title' => trim((string) ($page['meta_title'] ?? '')) ?: trim((string) ($electricConfig['meta']['meta_title'] ?? '')),
            'description' => trim((string) ($page['meta_description'] ?? '')) ?: trim((string) ($electricConfig['meta']['meta_description'] ?? '')),
            'canonical' => route_url($route),
        ];

        $primaryHref = $resolveHref((string) ($electricConfig['hero']['primary_url'] ?? ''));
        $secondaryHref = $resolveHref((string) ($electricConfig['hero']['secondary_url'] ?? ''));
        $trustButtonHref = $resolveHref((string) ($electricConfig['trust']['button_url'] ?? ''));
        $zonesMap = trim((string) ($electricConfig['zones']['map_image'] ?? ''));

        render_head($meta);
        render_header(route_url($route));
        ?>
        <section class="page-hero elec-page-hero">
            <div class="container elec-page-hero__inner">
                <div class="elec-page-hero__content">
                    <p class="eyebrow"><?= e($electricConfig['hero']['eyebrow']) ?></p>
                    <h1><?= e($electricConfig['hero']['title']) ?></h1>
                    <p class="elec-page-hero__lead"><?= e($electricConfig['hero']['lead']) ?></p>
                </div>
                <div class="elec-page-hero__actions">
                    <?php if ($primaryHref !== ''): ?>
                        <a class="btn btn--outline" href="<?= e($primaryHref) ?>"><?= e($electricConfig['hero']['primary_label']) ?></a>
                    <?php endif; ?>
                    <?php if ($secondaryHref !== ''): ?>
                        <a class="btn btn--primary" href="<?= e($secondaryHref) ?>"><?= e($electricConfig['hero']['secondary_label']) ?></a>
                    <?php endif; ?>
                </div>
            </div>
        </section>

        <section class="section section--soft elec-section">
            <div class="container">
                <div class="section-heading section-heading--center">
                    <h2><?= e($electricConfig['interventions']['title']) ?></h2>
                    <p><?= e($electricConfig['interventions']['lead']) ?></p>
                </div>
                <div class="elec-interventions-grid">
                    <?php foreach ((array) $electricConfig['interventions']['items'] as $item): ?>
                        <article class="card elec-intervention-card">
                            <div class="elec-intervention-card__icon"><?= e($item['icon']) ?></div>
                            <h3><?= e($item['title']) ?></h3>
                            <p><?= e($item['text']) ?></p>
                        </article>
                    <?php endforeach; ?>
        ...
        <section class="section elec-section">
            <div class="container">
                <div class="section-heading section-heading--center">
                    <h2><?= e($electricConfig['trust']['title']) ?></h2>
                    <p><?= e($electricConfig['trust']['lead']) ?></p>
                </div>
                <div class="card elec-trust-card">
                    <div class="elec-trust-grid">
                        <?php foreach ((array) $electricConfig['trust']['benefits'] as $benefit): ?>
                            <article class="elec-trust-item">
                                <h3><?= e($benefit['title']) ?></h3>
                                <p><?= e($benefit['text']) ?></p>
                            </article>
                        <?php endforeach; ?>
                    </div>
                    <div class="elec-price-box">
                        <p class="elec-price-box__label"><?= e($electricConfig['trust']['price_label']) ?></p>
                        <h3><?= e($electricConfig['trust']['price_value']) ?></h3>
                        <p><?= e($electricConfig['trust']['price_note']) ?></p>
                    </div>
                    <div class="elec-trust-footer">
                        <span><?= e($electricConfig['trust']['footer_text']) ?></span>
                        <?php if ($trustButtonHref !== ''): ?>
                            <a class="btn btn--primary" href="<?= e($trustButtonHref) ?>"><?= e($electricConfig['trust']['button_label']) ?></a>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <section class="section section--soft elec-section">
            <div class="container">
                <div class="section-heading section-heading--center">
                    <h2><?= e($electricConfig['steps']['title']) ?></h2>
                    <p><?= e($electricConfig['steps']['lead']) ?></p>
                </div>
                <div class="elec-steps-grid">
                    <?php foreach ((array) $electricConfig['steps']['items'] as $step): ?>
                        <article class="card elec-step-card">
                            <span class="elec-step-card__number"><?= e($step['number']) ?></span>
                            <h3><?= e($step['title']) ?></h3>
                            <p><?= e($step['text']) ?></p>
                        </article>
                    <?php endforeach; ?>
        ...
        <section class="section elec-section">
            <div class="container">
                <div class="section-heading section-heading--center">
                    <h2><?= e($electricConfig['zones']['title']) ?></h2>
                    <p><?= e($electricConfig['zones']['lead']) ?></p>
                </div>
                <div class="card elec-zones-card">
                    <?php if ($zonesMap !== ''): ?>
                        <div class="elec-zones-card__map"><img src="<?= e(asset_url($zonesMap)) ?>" alt="Zones d'intervention"></div>
                    <?php endif; ?>
                    <div class="elec-zones-grid">
                        <?php foreach ((array) $electricConfig['zones']['items'] as $zone): ?>
                            <div class="elec-zone-pill"><?= e($zone['label']) ?></div>
                        <?php endforeach; ?>
                    </div>
                </div>
            </div>
        ...
        <section class="section section--soft elec-section">
            <div class="container elec-faq-wrap">
                <div class="section-heading section-heading--center">
                    <h2><?= e($electricConfig['faq']['title']) ?></h2>
                </div>
                <div class="elec-faq-list">
                    <?php foreach ((array) $electricConfig['faq']['items'] as $faq): ?>
                        <details class="card elec-faq-item">
        ...
        <section class="section elec-section">
            <div class="container">
                <div class="section-heading section-heading--center">
                    <h2><?= e($electricConfig['contact']['title']) ?></h2>
                    <p><?= e($electricConfig['contact']['lead']) ?></p>
                </div>
                <div class="split-panel elec-contact-panel">
                    <div class="card form-card elec-contact-form-card">
                        <h3><?= e($electricConfig['contact']['form_title']) ?></h3>
                        <form action="<?= e(route_url('quote')) ?>" method="post">
                            <input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>">
                            <input type="hidden" name="form_type" value="quote">
                            <input type="text" name="website" value="" class="hp-field" tabindex="-1" autocomplete="off">
                            <div class="form-grid">
                                <label>Nom complet *<input type="text" name="full_name" required placeholder="Votre nom"></label>
                                <label>Téléphone *<input type="tel" name="phone" required placeholder="06 12 34 56 78"></label>
                            </div>
                            <div class="form-grid">
                                <label>Email<input type="email" name="email" placeholder="Votre email"></label>
                                <label>Ville<input type="text" name="city" placeholder="Meaux / Paris / 77"></label>
                            </div>
                            <label>Service souhaité<select name="service_type"><option value="<?= e($page['title']) ?>"><?= e($page['title']) ?></option><?php foreach (home_cards() as $card): ?><option value="<?= e($card['title']) ?>"><?= e($card['title']) ?></option><?php endforeach; ?></select></label>
                            <label>Votre besoin<textarea name="message" required placeholder="Expliquez rapidement votre panne ou votre demande"></textarea></label>
                            <button class="btn btn--primary btn--block" type="submit">Envoyer ma demande</button>
                        </form>
                    </div>
                    <div class="elec-contact-side">
                        <div class="card elec-contact-card">
                            <h3><?= e($electricConfig['contact']['info_title']) ?></h3>
                        ...
                        <div class="card elec-contact-card">
                            <h3><?= e($electricConfig['contact']['reasons_title']) ?></h3>
                            <ul class="elec-contact-reasons">
                                <?php foreach ((array) $electricConfig['contact']['reasons'] as $reason): ?>
                                    <li><?= e($reason['text']) ?></li>
                                <?php endforeach; ?>
                            </ul>
                        </div>
        ...
        <?php if (trim((string) ($page['content_html'] ?? '')) !== ''): ?>
            <section class="section elec-section elec-section--content">
                <div class="container">
